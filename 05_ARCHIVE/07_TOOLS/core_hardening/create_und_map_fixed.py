#!/usr/bin/env python3
"""
Create undetermined positions visualization (UND_MAP.svg) - FIXED VERSION
Shows which positions are determined by anchors only (via wheel propagation) vs need tail.
"""
...
def create_und_map_svg():
    ...
    for name, info in anchors.items():
        for i in range(info['start'], info['end'] + 1):
            ...
            if idx < len(info['plaintext']):
                anchor_constraints[i] = info['plaintext'][idx]
    
    # Solve wheels with anchor constraints only
    wheels = solve_wheels_baseline(ct, anchor_constraints)
    
    if wheels is None:
        print("ERROR: Could not solve wheels with anchors only!")
        return
    
    # Derive plaintext using solved wheels
    derived_pt = derive_pt_from_wheels(ct, compute_baseline_class, wheels)
    
    # Count undetermined positions (those with '?')
    undetermined = []
    determined = []
    for i in range(97):
        if derived_pt[i] == '?':
    ...
    print(f"Anchor constraints: {len(anchor_constraints)} positions")
    print(f"Determined by wheel propagation: {len(determined)} positions")
    print(f"Undetermined (need tail): {len(undetermined)} positions")
    ...
    for i in range(97):
        ...
        if i in determined:
            # Determined position - green circle
    ...
    legend_y = height - 20
    svg_lines.extend([
        f'  <circle cx="{width - 250}" cy="{legend_y}" r="5" fill="green" opacity="0.7"/>',
        f'  <text x="{width - 240}" y="{legend_y + 5}" font-size="12">Determined (anchors)</text>',
        f'  <circle cx="{width - 120}" cy="{legend_y}" r="5" fill="red" opacity="0.7"/>',
        f'  <text x="{width - 110}" y="{legend_y + 5}" font-size="12">Undetermined ({len(undetermined)})</text>',
    ])
    ...
    analysis = {
        'total_positions': 97,
        'anchor_constrained': len(anchor_constraints),
        'wheel_determined': len(determined),
...
- **Total positions**: 97
- **Anchor constraints**: {len(anchor_constraints)} positions (direct anchors)
- **Wheel-determined**: {len(determined)} positions (via propagation)
- **Undetermined**: {len(undetermined)} positions (need tail)
...
The four anchors constrain the wheels sufficiently to determine {len(determined)} of 97 positions through algebraic propagation. However, {len(undetermined)} positions remain undetermined, confirming the algebraic necessity of the tail for unique solution determination.
"""
    ...
    print(f"Summary: {len(determined)} determined by anchors, {len(undetermined)} undetermined (need tail)")
